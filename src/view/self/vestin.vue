            ...
            <van-grid :column-num="2" style="background-color:rgba(0, 0, 0, .1);">
              <van-grid-item v-for="(value,index) in newItems" :key="index" style="width:0;">
                <div class="photo" @click="goDetailPage(value.eid, value.departmentId)">
                  <van-image width="8rem" height="10rem" :src="imgSrc + value.eid">
                  </van-image>
                </div>
                <p class="personRole">姓名:{{value.name}}</p>
                <p @click="changeRole(value.role)">点击查看职责:</p>
                <van-popup position="bottom" :style="{ height: '30%' }" v-model="showRole" :overlay-style="{ backgroundColor: 'rgba(0, 0, 0,0.3)'}">
